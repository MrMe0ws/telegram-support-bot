...
Бот технической поддержки для VPN и других сервисов: клиент пишет в ЛС боту, сообщения попадают в **топики форум-супергруппы**, ответы саппорта возвращаются клиенту **от имени бота**. Поддерживаются медиа, блокировки, история в **SQLite**, **админ-панель** в ЛС (инлайн-кнопки), закрытие тикета командами **`/close`** (у клиента и у админа в теме), **`/profile`** в теме для админа, зеркалирование **реакций** между топиком и ЛС.
